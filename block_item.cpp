...
Block_item::Block_item(QGraphicsScene *parent, int y, int x, QString letter,
                         bool graphics_mode,
                         QColor tile_color) :
...
QRectF Block_item::boundingRect() const
{    
    return QRectF(0, 0, GAME_SQUARE_WIDTH, GAME_SQUARE_HEIGHT);
}

void Block_item::paint(QPainter *painter,
...
void Block_item::set_properties(int y, int x, QString letter, QColor tile_color)
{
    this->y = y;
    this->x = x;
    this->letter_ = letter;
